...
function getSemanasDelMes(refDate) {
  ...
  while (isBefore(cursor, mesEnd) || cursor.getTime() === mesEnd.getTime()) {
    ...
    semanas.push({
      ...
      isStarted:   !isAfter(semStart, hoy),
      isCurrent:   !isAfter(semStart, hoy) && !isBefore(semEnd, hoy),
      isCompleted: isBefore(semEnd, hoy) && !isToday(semEnd),
    })
...
function buildRankingPuntos(recs, configPuntos) {
  const map = {}
  recs.forEach((r) => {
    const all = [...new Set([...(r.descargadores ?? []), ...(r.estibadores ?? []), ...(r.workers ?? [])])]
    all.forEach((name) => {
      const key = name.trim().toLowerCase()
      if (!map[key]) map[key] = { name: name.trim(), puntos: 0, cajas: 0, descargas: 0, minutos: 0 }
      ...
      map[key].puntos    += cajas * factor
      map[key].cajas     += cajas
      map[key].descargas += 1
      map[key].minutos   += minutos
    })
  })
...
function buildConteoFinal(recs, configPuntos) {
  const map = {}
  recs.forEach((r) => {
    const all = [...new Set([...(r.descargadores ?? []), ...(r.estibadores ?? []), ...(r.workers ?? [])])]
    all.forEach((name) => {
      const key = name.trim().toLowerCase()
      if (!map[key]) map[key] = { name: name.trim(), cajas: 0, puntos: 0, descargas: 0 }
      const cajas  = getCajasWorker(r, name)
      const factor = getFactorCarga(r.tipo_carga || r.tipoCarga, configPuntos)
      map[key].cajas     += cajas
      map[key].puntos    += cajas * factor
  ...
  return Object.values(map)
    .map((w) => ({ ...w, cajas: Math.round(w.cajas), puntos: Math.round(w.puntos) }))
    .sort((a, b) => b.cajas - a.cajas)
}
// ── Sección mes anterior colapsable ─────────────────────────────────────────

function MesAnteriorSection({ records, mesAnterior, configPuntos, dark }) {
  const [open, setOpen] = useState(false)

  const recordsMesAnt = useMemo(() =>
    filtrarOperativos(records, mesAnterior.start, mesAnterior.end),
    [records, mesAnterior]
  )

  const rankingAnt   = useMemo(() => buildRankingPuntos(recordsMesAnt, configPuntos), [recordsMesAnt, configPuntos])
  const conteoAnt    = useMemo(() => buildConteoFinal(recordsMesAnt, configPuntos),   [recordsMesAnt, configPuntos])
  const totalCajas   = useMemo(() => recordsMesAnt.reduce((a, r) => a + (r.cajas_reales || r.cajasReales || 0), 0), [recordsMesAnt])
  const totalPuntos  = useMemo(() => recordsMesAnt.reduce((a, r) => {
    const f = getFactorCarga(r.tipo_carga || r.tipoCarga, configPuntos)
    return a + (r.cajas_reales || r.cajasReales || 0) * f
  }, 0), [recordsMesAnt, configPuntos])

  const labelMes = format(mesAnterior.start, 'MMMM yyyy', { locale: es })
  ...
  return (
    <div className="bg-white dark:bg-[#162050] rounded-2xl shadow border border-[#8fa3b1]/20 overflow-hidden">
      ...
      <button
        ...
        style={{ background: open ? 'linear-gradient(135deg, #0f2460 0%, #1a3a8f 100%)' : undefined }}>
        <div className="flex items-center gap-3">
          <div>
            <p className={`font-black text-sm capitalize ${open ? 'text-white' : 'text-slate-800 dark:text-white'}`}>
              📅 {labelMes}
            </p>
            <p className={`text-xs ${open ? 'text-white/70' : 'text-[#8fa3b1]'}`}>
              {recordsMesAnt.length} descargas · {Math.round(totalCajas).toLocaleString()} cajas · {Math.round(totalPuntos).toLocaleString()} pts
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
            open
              ...
              : 'bg-[#8fa3b1]/10 text-[#8fa3b1]'
          }`}>Mes anterior</span>
          {open
            ? <ChevronUp size={16} className={open ? 'text-white' : 'text-[#8fa3b1]'} />
            : <ChevronDown size={16} className="text-[#8fa3b1]" />}
        </div>
      </button>

      {open && recordsMesAnt.length === 0 && (
        <div className="text-center py-8 text-[#8fa3b1] text-sm">
          <div className="text-3xl mb-2">📅</div>
          <p>Sin actividad registrada en {labelMes}</p>
        </div>
      )}
      ...
      {open && recordsMesAnt.length > 0 && (
        <div className="space-y-4 p-4">
          ...
          {/* Ranking por puntos */}
          {rankingAnt.length > 0 && (
            <div className="rounded-2xl border border-[#8fa3b1]/20 overflow-hidden">
              <div className="px-4 py-3 border-b border-[#8fa3b1]/10"
                style={{ background: 'linear-gradient(135deg, #1a3a8f 0%, #2563c4 100%)' }}>
                <p className="text-white font-black text-sm capitalize">🏆 Ranking — {labelMes}</p>
                  ...
                  return (
                    <div key={w.name} className={`rounded-xl border p-3 ${
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-bold text-sm flex-1 text-slate-800 dark:text-white">{w.name}</span>
          ...
          {/* Conteo Final por cajas */}
          {conteoAnt.length > 0 && (
            <div className="rounded-2xl border border-[#8fa3b1]/20 overflow-hidden">
              <div className="px-4 py-3 border-b border-[#8fa3b1]/10"
                style={{ background: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)' }}>
                <p className="text-white font-black text-sm capitalize">�� Conteo Final — {labelMes}</p>
                <p className="text-white/70 text-xs">Suma del mes · ordenado por cajas</p>
              </div>
                  ...
                  <div className="flex items-end justify-center gap-3">
                    {conteoAnt[1] && (
                      <div className="flex-1 text-center">
                        <div className="bg-slate-100 dark:bg-slate-700 rounded-t-xl pt-3 pb-2 px-2">
                          ...
                          <p className="font-bold text-xs text-slate-700 dark:text-white truncate">{conteoAnt[1].name}</p>
                          <p className="font-black text-base text-slate-800 dark:text-white">{conteoAnt[1].cajas.toLocaleString()}</p>
                    ...
                    <div className="flex-1 text-center">
                      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-t-xl pt-3 pb-2 px-2">
                        <p className="text-2xl mb-1">🥇</p>
                        <p className="font-bold text-xs text-slate-700 dark:text-white truncate">{conteoAnt[0].name}</p>
                        <p className="font-black text-xl text-yellow-600 dark:text-yellow-400">{conteoAnt[0].cajas.toLocaleString()}</p>
                        <p className="text-[10px] text-[#8fa3b1]">cajas</p>
                      </div>
                    ...
                    {conteoAnt[2] && (
                      <div className="flex-1 text-center">
                        <div className="bg-orange-50 dark:bg-orange-900/10 rounded-t-xl pt-3 pb-2 px-2">
                          <p className="text-2xl mb-1">🥉</p>
                          <p className="font-bold text-xs text-slate-700 dark:text-white truncate">{conteoAnt[2].name}</p>
                          <p className="font-black text-base text-slate-800 dark:text-white">{conteoAnt[2].cajas.toLocaleString()}</p>
                          <p className="text-[10px] text-[#8fa3b1]">cajas</p>
                        </div>
              ...
              <div className="px-4 py-2 bg-[#8fa3b1]/5 border-t border-[#8fa3b1]/10">
                <p className="text-[10px] text-[#8fa3b1] capitalize">Conteo cerrado · {labelMes}</p>
              </div>
            </div>
          )}
        </div>
...
export default function MonthlyAnalytics({ records = [], dark, configPuntos }) {
  ...
  const semanasData = useMemo(() =>
    semanas.map((s) => {
      if (!s.isStarted) return { ...s, cajas: 0, puntos: 0, descargas: 0, hasData: false }
  ...
  const rankingMes = useMemo(() => buildRankingPuntos(recordsMesActual, configPuntos), [recordsMesActual, configPuntos])
  const conteoFinal = useMemo(() => buildConteoFinal(recordsMesActual, configPuntos), [recordsMesActual, configPuntos])
  ...
  return (
    <div className="space-y-4">
      ...
      {deltaPuntos !== null && (
        <div className={`rounded-xl px-4 py-2 text-xs font-semibold flex items-center gap-2 ${deltaPuntos >= 0 ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}>
          <span>{deltaPuntos >= 0 ? '↑' : '↓'}</span>
          <span>{Math.abs(deltaPuntos).toLocaleString()} pts {deltaPuntos >= 0 ? 'mas' : 'menos'} que {mesAnterior.label}</span>
        </div>
      )}

      <div className="bg-white dark:bg-[#162050] rounded-2xl shadow border border-[#8fa3b1]/20 overflow-hidden">
        <div className="px-4 py-3 border-b border-[#8fa3b1]/10" style={{ background: 'linear-gradient(135deg, #1a3a8f 0%, #2563c4 100%)' }}>
          <p className="text-white font-black text-sm capitalize">📅 {mesActual.label}</p>
          <p className="text-white/70 text-xs">Progreso por semana · solo dias operativos</p>
        </div>
        <div className="divide-y divide-[#8fa3b1]/10">
          {semanasData.map((s) => (
            <div key={s.num} className={`px-4 py-3 ${s.isCurrent ? 'bg-indigo-50 dark:bg-indigo-900/10' : ''}`}>
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full shrink-0 ${s.isCurrent ? 'bg-indigo-500 animate-pulse' : s.isCompleted ? 'bg-green-500' : 'bg-[#8fa3b1]/30'}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className={`font-bold text-sm ${s.isStarted ? 'text-slate-800 dark:text-white' : 'text-[#8fa3b1]'}`}>{s.label}</p>
                    {s.isCurrent && <span className="text-[10px] bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300 px-2 py-0.5 rounded-full font-semibold">En curso</span>}
                    {s.isCompleted && <span className="text-[10px] bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-300 px-2 py-0.5 rounded-full font-semibold">Completada</span>}
                  </div>
                  <p className="text-[11px] text-[#8fa3b1]">{s.sublabel}</p>
                </div>
                {s.isStarted ? (
                  <div className="flex gap-4 shrink-0 text-right">
                    <div><p className="font-black text-sm text-[#1a3a8f] dark:text-indigo-300">{s.puntos.toLocaleString()}</p><p className="text-[10px] text-[#8fa3b1]">pts</p></div>
                    <div><p className="font-bold text-sm text-slate-700 dark:text-white">{s.cajas.toLocaleString()}</p><p className="text-[10px] text-[#8fa3b1]">cajas</p></div>
                  </div>
                ) : <p className="text-xs text-[#8fa3b1]/50 shrink-0">Proximamente</p>}
              </div>
              {s.isCurrent && s.hasData && (
                <div className="mt-2 ml-5">
                  <div className="h-1.5 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden">
                    <div className="h-full rounded-full bg-indigo-500 transition-all duration-500"
                      style={{ width: Math.min(100, (s.cajas / Math.max(...semanasData.filter((x) => x.isStarted).map((x) => x.cajas), 1)) * 100) + '%' }} />
                  </div>
                </div>
              )}
            </div>
      ...
      {conteoFinal.length > 0 && (
        <div className="bg-white dark:bg-[#162050] rounded-2xl shadow border border-[#8fa3b1]/20 overflow-hidden">
          <div className="px-4 py-3 border-b border-[#8fa3b1]/10" style={{ background: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)' }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white font-black text-sm capitalize">🏁 Conteo Final — {mesActual.label}</p>
                <p className="text-white/70 text-xs">Suma de las 4 semanas · dias operativos · ordenado por cajas</p>
              </div>
              {!mesTerminado && <span className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded-full font-semibold">En curso</span>}
            </div>
          </div>
          ...
          <div className="divide-y divide-[#8fa3b1]/10 px-4 pb-3">
            {conteoFinal.map((w, i) => (
              <div key={w.name} className="flex items-center gap-3 py-2.5">
                <span className="text-base w-7 text-center shrink-0">{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : '#' + (i + 1)}</span>
                <span className="font-bold text-sm flex-1 text-slate-800 dark:text-white truncate">{w.name}</span>
                <div className="flex gap-4 shrink-0 text-right">
                  <div><p className="font-black text-sm text-[#7c3aed] dark:text-purple-300">{w.cajas.toLocaleString()}</p><p className="text-[10px] text-[#8fa3b1]">cajas</p></div>
                  <div><p className="font-bold text-sm text-slate-600 dark:text-slate-300">{w.puntos.toLocaleString()}</p><p className="text-[10px] text-[#8fa3b1]">pts</p></div>
                  <div><p className="font-bold text-sm text-slate-600 dark:text-slate-300">{w.descargas}</p><p className="text-[10px] text-[#8fa3b1]">desc.</p></div>
                </div>
              </div>
            ))}
          </div>
          <div className="px-4 py-2 bg-[#8fa3b1]/5 border-t border-[#8fa3b1]/10">
            <p className="text-[10px] text-[#8fa3b1]">{mesTerminado ? 'Conteo cerrado · ' + mesActual.label : 'Se actualiza automaticamente · ' + mesActual.label}</p>
          </div>
        </div>
      )}

      {/* Sección mes anterior colapsable */}
      <MesAnteriorSection records={records} mesAnterior={mesAnterior} configPuntos={configPuntos} dark={dark} />
